...
<div class="container">
    @if(isset($comment))
        ...
        @method('PUT')
    @elseif(isset($id))
        <form class="mt-2" name="create_platform" action="{{route('comments.store',$id)}}" method="POST" enctype="multipart/form-data">
    @endif
    @csrf
    ...
    @endif
    <input type="hidden" id="incident_id" name="incident_id">{{$id}}</input>
    </form>
</div>
@endsection